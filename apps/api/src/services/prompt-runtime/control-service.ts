...
export interface PromptRuntimeGovernanceEntry {
  sourceKind: string;
  declaredLevel?: import("@tavern/core").PromptRuntimeSourceGovernanceLevel;
  registered: boolean;
  effectiveRetention: import("@tavern/core").PromptRuntimeGovernanceRetention;
  pinned: boolean | null;
...
export interface PromptRuntimeGovernanceMismatch {
...
export interface PromptRuntimeGovernanceView {
...
export interface PromptRuntimeExplainDiff {
  ...
  governanceChanges: PromptRuntimeDiffEntry<unknown>[];
  limitations: string[];
}
  ...
  async getHistoricalExplain(floorId: string, accountId: string): Promise<PromptRuntimeHistoricalExplain> {
    ...
    if (snapshotRow) {
      ...
      const limitations = [
        ...
        ...PROMPT_RUNTIME_HISTORICAL_EXPLAIN_COMMON_LIMITATIONS,
        ...(snapshot.snapshotVersion === 1 ? ["This historical explain snapshot predates governance capture, so governance is returned as null."] : []),
      ];
      return {
        floor: {
          id: floor.id,
...
function mapPromptRuntimeExplainSnapshotRow(
  ...
  const sourceMapEnvelope = parsePromptRuntimeExplainSourceMapEnvelope({
    snapshotVersion: row.snapshotVersion,
    sourceMapJson: row.sourceMapJson,
  });
  ...
  return {
    ...
    snapshotVersion: sourceMapEnvelope.snapshotVersion,
...
function buildPromptRuntimeExplainDiff(args: {
  ...
  return {
    ...
    governanceChanges: buildPromptRuntimeDiffEntries(
      "governance",
      args.leftSnapshot.governance ?? null,
      args.rightSnapshot.governance ?? null,
    ),
    limitations,
  };
}

...
如需统一了解平台后台作业的状态、轮询、重试与取消语义，先看 [Background Jobs（后台作业）](./background-jobs)。

记忆系统在对话过程中会自动产生后台作业，例如提取摘要、压缩长摘要、定期维护。这些作业在后台排队执行，不阻塞对话。

## 什么时候需要看这页

- 你想知道某次聊天后的记忆提取任务有没有跑成功。
- 你要排查某个记忆作业为什么失败或一直停在等待状态。
- 你要手动重试失败作业，或者主动触发记忆维护。

## 一个简单例子

假设某个会话刚刚完成一轮聊天，但记忆没有按预期更新，可以按这个顺序排查：

1. `GET /memory/jobs?floor_id=...`：找到这次楼层相关的后台作业。
2. 看 `status`、`job_type`、`last_error`，确认它是成功、失败还是还在等待。
3. 如果作业已经进入 `dead_letter`，再调用重试接口重新排队。

## 先理解几个词

| 词 | 这里的意思 |
| ---- | ---- |
| 后台作业 | 由 worker 在后台执行的任务，不阻塞正常聊天 |
| `ingest_turn` | 从一次聊天结果里提取记忆 |
| `compact_macro` | 把多条短摘要压缩成更长、更少的摘要 |
| `rebuild_scope` | 重新整理某个作用域下的记忆状态 |
| scope | 记忆生效的范围，例如 `global`、`chat`、`branch`、`floor` |
